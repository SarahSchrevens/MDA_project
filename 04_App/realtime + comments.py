...
import re #regular expressions. It allows to search, match, and manipulate strings using patterns.
from functools import reduce #cumulative computation on a list, applying a function to pairs of elements from left to right until a single result is obtained 
import time # functions for working with time-related operations
import datetime
import gzip #compress and decompress files or data
import joblib #to save results
import urllib.request #fetching data from URLs, to open URLs, read data from them, and perform HTTP operations
import json #encode Python objects into JSON strings and decode JSON strings into Python objects
import io # classes for working with streams, files, and other input/output operations
import sklearn

#----------------------------------------------------------------------------------------------------------

# SAME AS NOTEBOOKS FOR SCRAPING
    ...
    if not selected_data.empty:
        ...
        trace = go.Scattermapbox(
            ...
            marker=dict(
                size=10,
                sizemode='diameter',
                color=predict.map({'Normal': 'blue', 'Busier than usual': 'red'}), #Map 'Normal' blue vs 'Busier than usual' red
            ),
...
app.layout = dbc.Container([
    ...
    dbc.Row([dcc.Markdown('**Real time data extraction** \U0001F4C5',
             ...
             html.Button('Click here to update', id='btn-nclicks-1'),  #creates a button component      
    ]),
...
#SAME AS past.py but now only input is button
...
#SAME AS past.py but instead of readings csv tbe data is created from scraping
